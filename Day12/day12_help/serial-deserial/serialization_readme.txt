...
Serializable i/f has no methods and is a marker(tag) interface. Its role is to provide a run time marker for allowing serialization/de serialization.
...
1.5 Verification ---Tries to match serial version UID
---doesn't match ---InvalidClassException --aborts de-serialization.
Matches --
2.  Class.newInstance()---empty (state inited to default)
